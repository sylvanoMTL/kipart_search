    ...
    return [
        ...
        # ── Phase 4 — Failure & Resilience ──
        {
            ...
            "steps": [
                ...
                "",
                "Expected: app has closed but installer did not run.",
                "The downloaded .exe remains in %TEMP% for manual retry.",
                "",
                "Relaunch the app manually:",
                "  PS> & 'C:\\Program Files\\KiPart Search\\kipart-search.exe'",
                f"  GUI> App starts normally on v{A} — no crash",
                "  GUI> Update check runs again → UpdateDialog reappears (can retry)",
            ],
        },
        {
            "id": "4.2",
            "phase": "Phase 4 — Failure & Resilience",
            "name": "Install Now Fallback (Launch Failure)",
            "story": "8.8",
            "steps": [
                "If os.startfile() fails (e.g. antivirus blocks the installer),",
                "the UpdateDialog should show an error with options:",
                "",
                "Simulate by temporarily renaming the downloaded .exe in %TEMP%:",
                "  PS> $f = Get-ChildItem $env:TEMP -Filter 'kipart-search-update-*.exe' | Select -First 1",
                "  PS> Rename-Item $f.FullName ($f.Name + '.bak')",
                "",
                "  PS> & 'C:\\Program Files\\KiPart Search\\kipart-search.exe'",
                "  GUI> UpdateDialog → click 'Update Now' → download completes",
                "  GUI> Click 'Install Now'",
                "  GUI> Error dialog should appear with options:",
                "       - Copy installer path to clipboard",
                "       - Open Release Page in browser",
                "       - Close",
                "",
                "Restore the file after testing:",
                "  PS> Rename-Item ($f.FullName + '.bak') $f.Name",
            ],
        },
        {
            "id": "4.3",
            ...
            "steps": [
                ...
                "  GUI> No startup popup about updates (network unavailable)",
                "  GUI> Log panel → no scary error messages (a quiet 'update check",
                "       failed' log entry is acceptable)",
                "  GUI> Search still works for JLCPCB offline database",
                "",
        ...
        },
        {
            ...
            "story": "8.8",
            "steps": [
                "This test verifies that stale partial download files are cleaned up.",
                "",
                "Create a fake old .partial file (simulating a failed download from 2 days ago):",
                ...
                "",
                "Launch the app (it cleans up stale .partial files on startup):",
                "  PS> & 'C:\\Program Files\\KiPart Search\\kipart-search.exe'",
        ...
        },
        {
            ...
            "story": "8.5",
            "steps": [
                "Ensure app has been launched at least once (to populate the cache).",
                "",
                "Relaunch immediately:",
                "  PS> & 'C:\\Program Files\\KiPart Search\\kipart-search.exe'",
                "  GUI> Log panel → should say 'using cached update check' or similar",
                ...
                "Relaunch:",
                "  PS> & 'C:\\Program Files\\KiPart Search\\kipart-search.exe'",
                "  GUI> Log panel → should say 'checking for updates' (fresh API call)",
                "  GUI> Close app",
            ],
        },
        {
